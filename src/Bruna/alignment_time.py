...
from evaluation import shared_model, online_shared, test_time
...
def main(args):
    ...
    results = test_time(dataset, paradigm, pipes, config)

    print(results.head())

    # Save results
    results.to_csv(f"{run_dir}/{experiment_name}_align-time.csv")
