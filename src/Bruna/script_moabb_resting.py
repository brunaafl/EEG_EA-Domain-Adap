...
from src.Bruna.transform import TransformRRA
...
def main(args):
    ...
    size = config.alignment.size
    # Hardcoded for now, but think more
    if args.dataset == 'BNCI2014001':
        dataset.interval = [2, 7.5]

    t_break = input_window_samples

    euclidean_rest = TransformRRA(t_break, size=size, dtype='raw')

    eegnet_RRA = Pipeline([('align', euclidean_rest), ('transform', transform), ('clf', clone(clf))])
    pipes[f'{config.model.type}_{dataset.code}_RRA-{size}'] = eegnet_RRA
