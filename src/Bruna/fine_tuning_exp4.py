...
def main(args):
    ...
    if args.ea == 'alignment':
        ...
        if config.model.type == "EEGNetv4":
            run = f'/mnt/beegfs/home/aristimunha/bruna/EEG_EA-Domain-Adap/output/runs/EpochsFalse/indiv_m1_final-{args.dataset}-alignment' \
                  '-exp_4-0-both'
        elif config.model.type == "ShallowFBCSPNet":
            run = f'/mnt/beegfs/home/aristimunha/bruna/EEG_EA-Domain-Adap/output/runs/EpochsFalse/indiv_m2_final-{args.dataset}-alignment' \
                  '-exp_4-0-both'
    else:
        pipes[f"{config.model.type}_Without_EA"] = pipe
        if config.model.type == "EEGNetv4":
            run = f'/mnt/beegfs/home/aristimunha/bruna/EEG_EA-Domain-Adap/output/runs/EpochsFalse/indiv_m1_final-{args.dataset}-no' \
                  '-alignment-exp_4-0-both'
        elif config.model.type == "ShallowFBCSPNet":
            run = (f'/mnt/beegfs/home/aristimunha/bruna/EEG_EA-Domain-Adap/output/runs/EpochsFalse/indiv_m2_final-{args.dataset}-no'
